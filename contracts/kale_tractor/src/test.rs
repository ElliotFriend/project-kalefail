...
fn test_with_all_harvestable_pails() {
    ...
    let farmer = Address::from_str(&env, &"GBQZZ54DGBXZ6USGU5PMMAJVDLALGEMXXZEZC5BDDSAG7FI653DLMVTN");
    let rewards = tractor_client.harvest(&farmer, &vec![&env, 11692, 11697, 11698, 11699, 11700, 11701]);
...
fn test_with_mixed_harvestable_pails() {
    ...
    let farmer = Address::from_str(&env, &"GBQZZ54DGBXZ6USGU5PMMAJVDLALGEMXXZEZC5BDDSAG7FI653DLMVTN");
    let rewards = tractor_client.harvest(&farmer, &vec![&env, 0, 1, 2, 3, 11692, 11697, 11698, 11699, 11700, 11701]);
