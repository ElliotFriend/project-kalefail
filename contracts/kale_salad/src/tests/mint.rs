...
fn test_can_mint() {
    ...
    let [(_, broc), (_, cabb), (_, kohl), (_, brsp)] =
        fixture.vegetables;
...
fn test_cannot_mint_with_small_payment() {
    let fixture = TestFixture::create();
...
#[test]
fn test_can_mint_with_large_payment() {
    let fixture = TestFixture::create();
    ...
    assert_eq!(cabb.balance(&owner), 950 * 10_000_000);
    assert_eq!(kohl.balance(&owner), 950 * 10_000_000);
    assert_eq!(brsp.balance(&owner), 950 * 10_000_000);
}
