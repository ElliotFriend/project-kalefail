...
export const GET: RequestHandler = async ({ params }) => {
    ...
    const kaleContract = new Contract('CDL74RF5BLYR2YBLCCI7F5FB6TPSCLKEJUBSD2RSVWZ4YHF3VMFAIGWA');
    const { entries: instanceEntries } = await rpc.getLedgerEntries(kaleContract.getFootprint());
    const farmIndex = instanceEntries[0].val
        .contractData()
        .val()
        .instance()
        .storage()
    ...
    const { entries: pailEntries1 } = await rpc.getLedgerEntries(...possiblePails);

    // search for the next batch of 200 pails
    possiblePails = createPailLedgerKeys(searchIndex - 200, params.address);
    const { entries: pailEntries2, latestLedger } = await rpc.getLedgerEntries(...possiblePails);

    // mash the two together
    const pailEntries = [...pailEntries1, ...pailEntries2];

    const harvestablePails = pailEntries.map((e) => {
        const pail = scValToNative(e.val.contractData().val());
        if (
