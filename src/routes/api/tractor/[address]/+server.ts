...
export const GET: RequestHandler = async ({ params }) => {
    ...
    let farmIndex = instanceEntries[0].val.contractData().val().instance().storage()?.find(e => scValToNative(e.key())[0] === 'FarmIndex')
    ...
    let possiblePails = Array.from({ length: 200 }, (_, i) => searchIndex - i).map(p => (
        xdr.LedgerKey.contractData(
            new xdr.LedgerKeyContractData({
                contract: new Address('CDL74RF5BLYR2YBLCCI7F5FB6TPSCLKEJUBSD2RSVWZ4YHF3VMFAIGWA').toScAddress(),
                key: nativeToScVal([
                    nativeToScVal('Pail', { type: 'symbol' }),
                    nativeToScVal('CC4RLF6LVFPDMPTWYB2NR7HEUTEDIHZX7WQZOJLEIWOE6Z36EPBCSXHQ', { type: 'address' }),
