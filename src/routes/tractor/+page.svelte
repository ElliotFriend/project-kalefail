<script lang="ts">
    ...
    let somePailsSelected = $derived(selectedPails.length > 0 && selectedPails.length < harvestablePails.length)
...
<PageHeader title="The Tractor" subtitle="Welcome to the heavy duty tractor. Where all your harvest dreams come true!" />

<p>Sometimes, stuff happens. Your webminer acts up, you have to log out, your dog pukes everywhere. Ya know: stuff! It's easy to potentially lose out on your hard-earned <code class="code">KALE</code> harvests. Well, fret no more!</p>

<p>The tractor is here to help you reclaim your harvest. You still have to get them before they're evicted (they haven't paid the landlord in a while!), but now you can get back to them easier then ever.</p>
...
<div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
    <div class="flex flex-col card bg-initial justify-between">
        ...
        <footer class="card-footer text-center">
            <button class="btn variant-filled" disabled={isFetching || !farmerAddress} onclick={fetchPails}>Fetch Pails</button>
        </footer>
    </div>
    <div class="card bg-initial">
        ...
        <section class="p-4 space-y-4">
            {#if harvestablePails.length}
                <p>Please select which pails of <code class="code">KALE</code> you'd like to harvest:</p>

                <div class="space-y-2">
                    <label class="flex items-center space-x-2">
                        <input class="checkbox" type="checkbox" onchange={togglePails} checked={allPailsSelected} indeterminate={somePailsSelected} />
